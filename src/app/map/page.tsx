...
const fetchToilets = async () => {
  try {
    const totalCountRes = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/seoul-toilets/1/1`);
    ...
    for (let startRowNum = 1; startRowNum <= totalCount; startRowNum += MAX_ROW_DATA_PER_CALL) {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/seoul-toilets/${startRowNum}/${endRowNum}`,
      );
